...
import progressbar
from scipy.sparse import coo_array as sp_array
...
def setup_collective_spin_vectors(ntls):
    ...
    all_m_index=[[],[[0,1]]]
    if SPARSE:
        all_spinrep=[[sp_array([])], [[sp_array([1,0],(2,)), sp_array([0,1],(2,))]]]
    else:
        all_spinrep=[[],[[array([1,0]),array([0,1])]]]
    #all_spinrep=[[COO], [[sp_array([1,0],(2,)), sp_array([0,1],(2,))]]]
    
    
    # Iterate over ns and build using previous expressions.
    pbar = progressbar.ProgressBar(maxval=ntls-1, widgets=widgets)
    pbar.start()
    for ns in range(2,ntls+1):
        ...
        for s in range(len(new_string_set)):
                    ...
                    for m_new in range(2):
                        ...
                        if (m_old>=0 and m_old<=ns-1):
                            ...
                            if(k_old>-1):                   
                                ...
                                # according to m_new
                                vec=zeros(2**ns)
                                rhs = all_spinrep[ns-1][s_old][k_old]
                                if SPARSE:
                                    rhs = rhs.todense()
                                vec[m_new::2]=rhs
                                #print(vec)
                                #print(all_spinrep[ns-1][s_old][k_old].todense(),vec)
            ...
            #print(new_spinreps_k)
            if SPARSE:
                new_spinreps_k = [sp_array(x, (x.size,)) for x in new_spinreps_k]
